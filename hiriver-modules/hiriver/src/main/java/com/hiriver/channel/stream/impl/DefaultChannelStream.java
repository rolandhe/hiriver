...
public class DefaultChannelStream implements ChannelStream {
	...
	@Override
	public void start() {
		context.setNextPos(loadBinlogPosistion());
	...
	private void providerThreadCore() {
		while (true) {
			...
			if (transactionRecognizer.isEnd(validOutput)) {
				...
				BufferableBinlogDataSet bufferDs = createPersistPosBufferableBinlogDataSet(
						transactionRecognizer.getCouldNextPos());
				ensureDispatch(bufferDs);
	...
	private BufferableBinlogDataSet convert(ValidBinlogOutput validOutput) {
		BinlogDataSet ds = new BinlogDataSet(this.channelId, streamSource.getHostUrl(),
				transactionRecognizer.getGTId());
	...
	private BinlogPosition loadBinlogPosistion() {
		if (context.getNextPos() != null) {
			LOG.info("load binlog position from mem,channelId is {}.", this.channelId);
